...
#
# Modified by pcarr  (get_section_data returns address and count. 
#                     if you pass None to __init__,output is returned in a 
#                     string.
#                     )
...
class ReadElf(object):
    ...
    def display_program_headers(self, show_heading=True):
        ...
        if show_heading:
            ...
            self._emit('Entry point is %s' %
                self._format_hex(elfheader['e_entry']))
            if machine == "ARM":
                if (elfheader['e_entry'] & 1):
                    self._emitline(' (Thumb mode)')
                else:
                    self._emitline(' (ARM Mode)')
            else:
                self._emitline()
            # readelf weirdness - why isn't e_phoff printed as hex? (for section
            # headers, it is...)
    ...
    def get_symbol_dictionary(self):
        ...
        for section in self.elffile.iter_sections():
            ...
            if section['sh_entsize'] == 0:
                """Symbol table has a sh_entsize of zero"""
                continue
            ...
            for nsym, symbol in enumerate(section.iter_symbols()):
                # symbol names are truncated to 25 chars, similarly to readelf 
                addr = int(symbol['st_value']) & 0xfffffffe  # clear thumb addr
                info = describe_symbol_type(symbol['st_info']['type'])
                if (info == 'FUNC') or (info == 'NOTYPE'):  
                    # they all seem to be NOTYPE in elf
                    #describe_symbol_bind(symbol['st_info']['bind'])
                    #describe_symbol_visibility(symbol['st_other']['visibility'])
                    #describe_symbol_shndx(symbol['st_shndx'])
    ...
    def get_section_data(self, section_spec):
        """ Return  a section. section_spec is either a section
            number or a name.
        """
        self.res=""
